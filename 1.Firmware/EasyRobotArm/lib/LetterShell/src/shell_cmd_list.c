...
/*****用户自定义添加的指令*****/
extern void shellServoTest(int id);
extern void shellForwardKinematicsControl(int A,int B,int C);
extern void shellForwardKinematicsControl2(int A,int B,int C);
extern void shellBackwardKinematicsControl(int X,int Y,int Z);
extern void shellBackwardKinematicsControl2(int X,int Y,int Z);
extern void shellJawControl(int X,int Y,int Z);
/****************************/
...
const ShellCommand shellCommandList[] = 
{
    ...
    /*用户添加的指令*/
    SHELL_CMD_ITEM(SHELL_CMD_PERMISSION(0)|SHELL_CMD_TYPE(SHELL_TYPE_CMD_FUNC)|SHELL_CMD_DISABLE_RETURN,
                st, shellServoTest, Servo Test),
    SHELL_CMD_ITEM(SHELL_CMD_PERMISSION(0)|SHELL_CMD_TYPE(SHELL_TYPE_CMD_FUNC)|SHELL_CMD_DISABLE_RETURN,
                fk, shellForwardKinematicsControl, Forward Kinematics Control),
    SHELL_CMD_ITEM(SHELL_CMD_PERMISSION(0)|SHELL_CMD_TYPE(SHELL_TYPE_CMD_FUNC)|SHELL_CMD_DISABLE_RETURN,
                fk2, shellForwardKinematicsControl2, Forward Kinematics Control2),
    
    SHELL_CMD_ITEM(SHELL_CMD_PERMISSION(0)|SHELL_CMD_TYPE(SHELL_TYPE_CMD_FUNC)|SHELL_CMD_DISABLE_RETURN,
            bk, shellBackwardKinematicsControl, Backward Kinematics Control),
    SHELL_CMD_ITEM(SHELL_CMD_PERMISSION(0)|SHELL_CMD_TYPE(SHELL_TYPE_CMD_FUNC)|SHELL_CMD_DISABLE_RETURN,
                bk2, shellBackwardKinematicsControl2, Backward Kinematics Control2),

    SHELL_CMD_ITEM(SHELL_CMD_PERMISSION(0)|SHELL_CMD_TYPE(SHELL_TYPE_CMD_FUNC)|SHELL_CMD_DISABLE_RETURN,
                jc, shellJawControl, Jaw Control),
};
